...
#define Vec4f CommonUtilities::Vector4f
...
struct ParticleEmitterData
{
	...
	Vec2f Position;
	Vec2f Dimensions;
	Vec2f MinParEmissionVel = { -150.f, -150.f };
	Vec2f MaxParEmissionVel = { 150.f, 150.f };
};
...
struct ParticleData
{
	...
	Vec2f Position[COUNT];
	Vec2f Velocity[COUNT];
	Vec4f Color[COUNT];
	StaticSprite* Sprite[COUNT];
};
...
class ParticleEmitter
{
	public:
		...
		void Activate(Vec2f aPosition);
		void LoadEmitterSettings(const std::string& aFileName);
		void SetPosition(const Vec2f aPos);
		void ResetEmitter();
		void SetEmitterSize(Vec2f aSize);
		void Render() const;
...
void ParticleEmitter<COUNT>::ResetParticle(unsigned int aIndex)
{
	...
	myParticleData.Color[aIndex].x = myData.MinParRed + static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / (myData.MaxParRed - myData.MinParRed)));
	myParticleData.Color[aIndex].y = myData.MinParGreen + static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / (myData.MaxParGreen - myData.MinParGreen)));
	myParticleData.Color[aIndex].z = myData.MinParBlue + static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / (myData.MaxParBlue - myData.MinParBlue)));
	myParticleData.Color[aIndex].w = 0.f;
...
template<unsigned int COUNT>
void ParticleEmitter<COUNT>::Activate(Vec2f aPosition)
{
...
void ParticleEmitter<COUNT>::UpdateParticle(unsigned int aIndex, CU::Time aTime)
{
	...
	myParticleData.Color[aIndex].w = newAlpha;
	myParticleData.Sprite[aIndex]->SetColor(myParticleData.Color[aIndex]);
...
void ParticleEmitter<COUNT>::LoadEmitterSettings(const std::string& aFileName)
{
	...
	for (unsigned int i = 0; i < COUNT; ++i)
	{
		myParticleData.Sprite[i] = new StaticSprite();
		myParticleData.Sprite[i]->Init(spritePath, false);
		myParticleData.Sprite[i]->SetLayer(enumRenderLayer::eGameObjects);
		ResetParticle(i);
		myParticleData.Sprite[i]->SetPivotWithPixels({0.5f, 0.5f});
	}
...
template<unsigned int COUNT>
void ParticleEmitter<COUNT>::SetPosition(const Vec2f aPos)
{
...
template<unsigned int COUNT>
Vec2f ParticleEmitter<COUNT>::GenerateNewParticlePos()
{
	Vec2f returnVec;
...
template<unsigned int COUNT>
void ParticleEmitter<COUNT>::SetEmitterSize(Vec2f aSize)
{
	myData.Dimensions = aSize;
}

template<unsigned int COUNT>
void ParticleEmitter<COUNT>::Render() const
{
	for (unsigned int i = 0; i < COUNT; ++i)
	{
		if (myParticleData.IsActive[i] == true)
		{
			myParticleData.Sprite[i]->Draw(myParticleData.Position[i]);
		}
	}
}